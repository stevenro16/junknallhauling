...
@if(count($employees ?? []) > 0)
    @php($selfId = session('admin_id'))
    {{-- Current admin first, then other employees, then Everyone last. --}}
    @php($ordered = collect($employees)->sortBy(fn ($e) => $e['id'] === $selfId ? 0 : 1)->values())
    <div class="flex flex-wrap items-center gap-2 mb-4">
        <span class="text-xs text-gray-500 mr-0.5">Show:</span>
        @foreach($ordered as $emp)
