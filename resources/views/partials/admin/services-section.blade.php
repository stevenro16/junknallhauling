      ...
      <div class="hidden md:block overflow-x-auto">
        <table class="w-full min-w-[760px] text-sm text-gray-200">
            <thead class="text-xs uppercase tracking-wider text-gray-400 border-b border-charcoal-600">
                <tr><th class="text-left py-2">Service Name</th><th class="text-left py-2">Price</th><th class="text-left py-2">Duration</th><th class="text-left py-2">Active</th><th class="text-left py-2">Customer</th><th class="text-left py-2">Agreement</th><th class="text-left py-2">Instructions</th><th class="text-right py-2">Actions</th></tr>
            </thead>
            <tbody>
                <template x-for="s in services" :key="s.id">
                    <tr class="border-b border-charcoal-700/60" :class="!s.active && 'opacity-50'">
                        ...
                        <td class="py-2">
                            <span x-show="editingId !== s.id" x-text="agreementName(s) || '—'" class="text-xs text-amber-300/90"></span>
                            <select x-show="editingId === s.id" x-model="ed.agreement_id" class="input-dark py-1 text-sm" x-cloak>
                                <option value="">— None —</option>
                                <template x-for="a in agreements" :key="a.id"><option :value="a.id" x-text="a.title"></option></template>
                            </select>
                        </td>
            ...
            <div class="rounded-lg border border-charcoal-700 p-3" :class="!s.active && 'opacity-60'">
                ...
                <template x-if="editingId !== s.id">
                    <div>
                        ...
                        <div x-show="agreementName(s)" x-cloak class="mt-1.5 text-xs text-amber-300/90">Agreement: <span x-text="agreementName(s)"></span></div>
